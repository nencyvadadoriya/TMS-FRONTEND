...
const DashboardPage = () => {
  ...
  const canBulkAddTaskTypes = useMemo(
    () => hasAccess("task_type_bulk_add"),
    [hasAccess],
  );
  const canBulkAddCompanies = useMemo(
    () => hasAccess("company_bulk_add"),
    [hasAccess],
  );
  const canBulkAddBrands = useMemo(
    () => hasAccess("brand_bulk_add"),
    [hasAccess],
  );
  ...
  const canEditTask = useCallback(
    (task: Task): boolean => {
      ...
      if (
        isMdImpexTask &&
        (role === "md_manager" || role === "ob_manager" || role === "manager")
      )
        return true;
    ...
    async (userId: string, updatedData: Partial<UserType>) => {
      ...
      if (!allowedByPerms) {
        ...
        if (requesterRoleKey === "md_manager") {
          if (targetRoleKey !== "manager" && !isAssistantLike(targetRoleKey))
  ...
  const validateForm = useCallback(
    (data?: NewTaskForm) => {
      ...
      // Company validation
      if (
        !formToValidate.companyName ||
        formToValidate.companyName.trim() === ""
      ) {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                      {stats.map((stat) => (
                        <button
                          ...
                          className={`bg-white p-6 rounded-2xl shadow-sm border-2 cursor-pointer transition-all duration-200 hover:shadow-md hover:-translate-y-1 relative ${
                        ...
                        >
                          <div
                            className={`absolute top-4 right-4 h-5 w-5 rounded-full border-2 transition-colors ${
                              selectedStatFilter === stat.id
                                ? "border-blue-600"
                                : "border-gray-300"
                            }`}
                          ...
                          {(() => {
                            ...
                            return (
                              <button
                                ...
                                className={`bg-white p-6 rounded-2xl shadow-sm border-2 transition-all duration-200 hover:shadow-md ${
                            ...
                            {canSendReminderForTask(task) && (
                              <button
                                ...
                                className={`px-3 py-2 rounded-lg border transition-colors ${
                                  sendingReminderByTaskId[String(task.id || "")]
