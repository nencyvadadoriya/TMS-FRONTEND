...
const DashboardPage = () => {
    ...
    const [showMdImpexAddTaskModal, setShowMdImpexAddTaskModal] = useState(false);
    ...
    const [showManagerAddBrandModal, setShowManagerAddBrandModal] =
        useState(false);
    ...
    const [sendingReminderByTaskId, setSendingReminderByTaskId] = useState<
    ...
    const [commentSidebarTask, setCommentSidebarTask] = useState<Task | null>(
        null,
    );
    const [commentDraft, setCommentDraft] = useState("");
    const [commentSidebarLoading, setCommentSidebarLoading] = useState(false);
    const [commentSidebarLoadingComments, setCommentSidebarLoadingComments] =
        useState(false);
    const [commentSidebarCommentsByTaskId, setCommentSidebarCommentsByTaskId] =
        useState<Record<string, CommentType[]>>({});
    const [speedEcomReassignTask, setSpeedEcomReassignTask] =
        useState<Task | null>(null);
    const [isSpeedEcomReassignSubmitting, setIsSpeedEcomReassignSubmitting] =
        useState<boolean>(false);
    const [loading, setLoading] = useState(true);
    const [searchTerm, setSearchTerm] = useState("");
    const [taskPage, setTaskPage] = useState(1);
    const [reviewsMonth, setReviewsMonth] = useState<string>(() =>
        monthKeyOfDate(new Date()),
    );
    const [reviewedTasksForSummary, setReviewedTasksForSummary] = useState<
        Task[]
    >([]);
    ...
    const userMappingsFetchInFlightRef = useRef<Map<string, Promise<void>>>(
        new Map(),
    );
    ...
    const employeeOfTheMonth = useMemo(() => {
        ...
        const currentUserCompany = String(
            (currentUser as any)?.companyName || (currentUser as any)?.company || "",
        )
            .trim()
            .toLowerCase();
        const isMdImpexUser =
        ...
        if (isMdImpexUser) {
            reviewedData = (tasks || []).filter((t) => {
                ...
                return (
                    reviewedAt >= monthRange.start && reviewedAt < monthRange.endExclusive
                );
            });
        }
        ...
        reviewed.forEach((t) => {
            ...
            const email = String(
                ...
                (typeof (t as any).assignedTo === "string"
                    ? (t as any).assignedTo
            ...
            if (monthRange) {
                ...
                if (
                    createdAt < monthRange.start ||
                    createdAt >= monthRange.endExclusive
                )
                    return;
            }
        ...
        const summaryRowsBase = rows.slice(0, 10).map((r) => {
            ...
            return {
                ...
                totalTasksReceived: (tasks || []).filter((t: any) => {
                    const rowEmail = String((r as any)?.email || "")
                        .trim()
                        .toLowerCase();
                    ...
                    if (!assignedToEmail || !rowEmail || assignedToEmail !== rowEmail)
                        return false;
                    if (!monthRange) return true;
                    const createdAtRaw = (t as any).createdAt || (t as any).assignedAt;
                    ...
                    return (
                        createdAt >= monthRange.start && createdAt < monthRange.endExclusive
                    );
                }).length,
        ...
        return {
            ...
            totalReviews: top
                ? (summaryRowsBase.find((r) => r.email === top.email)?.total ?? 0)
                : (rows[0]?.total ?? 0),
            totalTasksReceived: top
                ? (tasks || []).filter((t: any) => {
                    const topEmail = String((top as any)?.email || "")
                        .trim()
                        .toLowerCase();
                    const assignedToEmail =
                        normalizeEmailForMatch((t as any)?.assignedTo) ||
                        normalizeEmailForMatch((t as any)?.assignedToUser?.email) ||
                        normalizeEmailForMatch((t as any)?.assignedToUser);
                    if (!assignedToEmail || !topEmail || assignedToEmail !== topEmail)
                        return false;
                    if (!monthRange) return true;
                    const createdAtRaw = (t as any).createdAt || (t as any).assignedAt;
                    if (!createdAtRaw) return false;
    ...
    const pendingManagerReviewTasks = useMemo(() => {
        const normalizeEmailSafe = (v: unknown): string =>
            String(v || "")
                .trim()
                .toLowerCase();
        const normalizeRoleKey = (v: unknown): string =>
            String(v || "")
                .trim()
                .toLowerCase()
                .replace(/[\s-]+/g, "_");
        ...
        const getAssignedByEmail = (t: any): string => {
            ...
            if (raw && typeof raw === "object")
                return normalizeEmailSafe((raw as any)?.email);
        ...
        const getAssigneeRoleKey = (t: any): string => {
            ...
            const found = (users || []).find(
                (u: any) => normalizeEmailSafe((u as any)?.email) === email,
            );
        ...
        const list = (tasks || [])
            ...
            .sort((a: any, b: any) => {
                const aDate = new Date(
                    (a as any)?.statusUpdatedAt ||
                    (a as any)?.updatedAt ||
                    (a as any)?.createdAt ||
                    0,
                ).getTime();
                const bDate = new Date(
                    (b as any)?.statusUpdatedAt ||
                    (b as any)?.updatedAt ||
                    (b as any)?.createdAt ||
                    0,
                ).getTime();
    ...
    const reviewModalTask = useMemo(() => {
        ...
        return (
            (pendingManagerReviewTasks || []).find(
                (t: any) =>
                    String((t as any)?.id || (t as any)?._id || "") === reviewModalTaskId,
            ) || null
        );
    ...
    useEffect(() => {
        ...
        setReviewModalTaskId((prev) => {
            if (
                prev &&
                next.some(
                    (t: any) => String((t as any)?.id || (t as any)?._id || "") === prev,
                )
            )
                return prev;
    ...
    useEffect(() => {
        ...
        next.forEach((t: any) => {
            ...
            const assigneeName = String(
                (t as any)?.assignedToUser?.name || "",
            ).trim();
            const title = String((t as any)?.title || "Task").trim();
            toast.success(
                `Task completed${assigneeName ? ` by ${assigneeName}` : ""}: ${title}. Review required.`,
            );
        });
    }, [pendingManagerReviewTasks]);
    ...
    useEffect(() => {
        ...
        setLoading(true);
        void dispatch(fetchTasksThunk({ force: true })).finally(() => {
            setLoading(false);
        });
    ...
    useEffect(() => {
        ...
        const normalizeIncomingTask = (task: any) => {
            ...
            return {
                ...
                brand: (typeof task?.brand === "string"
                    ? task.brand
                    : task?.brand?.name || ""
                ).toString(),
            };
            ...
            try {
                ...
                try {
                    window.dispatchEvent(
                        new CustomEvent("taskUpdated", { detail: { task } }),
                    );
                } catch {
                    // ignore
                }
        ...
        const onBrandUpserted = (payload: any) => {
            try {
                ...
                const nextId = String(
                    (next as any)?.id || (next as any)?._id || "",
                ).trim();
                if (!nextId) return;
                setApiBrands((prev) => {
                    ...
                    const idx = list.findIndex(
                        (b: any) =>
                            String((b as any)?.id || (b as any)?._id || "").trim() === nextId,
                    );
                    if (idx >= 0) {
                        const clone = [...list];
                ...
                try {
                    window.dispatchEvent(
                        new CustomEvent("brandUpdated", { detail: { brandId: nextId } }),
                    );
                } catch {
                    // ignore
                }
        ...
        const onBrandDeleted = (payload: any) => {
            try {
                ...
                setApiBrands((prev) => {
                    ...
                    return list.filter(
                        (b: any) =>
                            String((b as any)?.id || (b as any)?._id || "").trim() !==
                            brandId,
                    );
                });
                brandsFetchedAtRef.current = Date.now();
                try {
                    window.dispatchEvent(
                        new CustomEvent("brandUpdated", { detail: { brandId } }),
                    );
                } catch {
                    // ignore
                }
        ...
        const onUserUpserted = (payload: any) => {
            try {
                ...
                setUsers((prev) => {
                    ...
                    const idx = list.findIndex(
                        (u: any) =>
                            String((u as any)?.id || (u as any)?._id || "").trim() === nextId,
                    );
                    if (idx >= 0) {
                        const clone = [...list];
                        clone[idx] = { ...(clone[idx] as any), ...(next as any) };
                ...
                try {
                    window.dispatchEvent(
                        new CustomEvent("userUpdated", { detail: { userId: nextId } }),
                    );
                } catch {
                    // ignore
                }
        ...
        const onUserDeleted = (payload: any) => {
            try {
                ...
                setUsers((prev) => {
                    ...
                    return list.filter(
                        (u: any) =>
                            String((u as any)?.id || (u as any)?._id || "").trim() !==
                            userIdToDelete,
                    );
                });
                usersFetchedAtRef.current = Date.now();
        ...
        const onAssignmentChanged = (payload: any) => {
            try {
                ...
                window.dispatchEvent(
                    new CustomEvent("assignmentsApplied", {
                        detail: {
                            companyName,
        ...
        const onReminderNew = (payload: any) => {
            try {
                ...
                setUnreadReminders((prev) => {
                    ...
                    const merged = [
                        next,
                        ...list.filter((x) => String(x?.id || "").trim() !== id),
                    ];
                    return merged;
                });
        ...
        const onCommentAdded = (payload: any) => {
            try {
                ...
                const actorEmail = String(comment?.userEmail || "")
                    .trim()
                    .toLowerCase();
                if (
                ...
                try {
                    window.dispatchEvent(
                        new CustomEvent("taskCommentUnread", { detail: { taskId } }),
                    );
                } catch {
                    // ignore
                }
        ...
        const onPermissionsUpdated = (payload: any) => {
            try {
    ...
    const hasAccess = useCallback(
        (moduleId: string) => {
            const role = String((currentUser as any)?.role || "")
                .trim()
                .toLowerCase();
            if (moduleId === "user_management")
                return role === "super_admin" || role === "admin";
            if (moduleId === "access_management" && (role === "am" || role === "rm"))
                return false;
            if (role === "super_admin" || role === "admin") return true;
            if (
                ["sbm", "rm", "am", "ar", "troubleshoot_manager"].includes(role) &&
                moduleId === "create_task"
            )
                return true;
            if (role === "troubleshoot_manager" && moduleId === "strike_page")
                return true;
            ...
            const perm = String((perms as any)[moduleId] || "")
                .trim()
                .toLowerCase();
    ...
    const canViewAllTasks = useMemo(() => {
        ...
        // If it's an MD Impex role, strictly respect the access logic
        if (role === "md_manager" || role === "ob_manager" || role === "troubleshoot_manager") {
    ...
    const canCreateTasks = useMemo(() => {
        const role = String((currentUser as any)?.role || "")
            .trim()
            .toLowerCase();
        // If it's an MD Impex role, check permissions from the access logic
        if (role === "md_manager" || role === "ob_manager" || role === "troubleshoot_manager") {
            return hasAccess("create_task");
        }
        return hasAccess("create_task");
    }, [currentUser, hasAccess]);
    const canBulkAddTaskTypes = useMemo(() => {
        const role = String((currentUser as any)?.role || "")
            .trim()
            .toLowerCase();
        // If it's an MD Impex role, strictly respect the access logic
        if (role === "md_manager" || role === "ob_manager" || role === "troubleshoot_manager") {
            return hasAccess("task_type_bulk_add");
        }
        return hasAccess("task_type_bulk_add");
    }, [currentUser, hasAccess]);
    const canBulkAddCompanies = useMemo(() => {
        const role = String((currentUser as any)?.role || "")
            .trim()
            .toLowerCase();
        // If it's an MD Impex role, strictly respect the access logic
        if (role === "md_manager" || role === "ob_manager" || role === "troubleshoot_manager") {
            return hasAccess("company_bulk_add");
        }
        return hasAccess("company_bulk_add");
    }, [currentUser, hasAccess]);
    const canBulkAddBrands = useMemo(() => {
        ...
        // If it's an MD Impex role, strictly respect the access logic
        if (role === "md_manager" || role === "ob_manager" || role === "troubleshoot_manager") {
            return hasAccess("brand_bulk_add");
        }
        return hasAccess("brand_bulk_add");
    }, [currentUser, hasAccess]);
    ...
    const isAdminLike = useMemo(() => {
        ...
        return (
            role === "admin" ||
            role === "super_admin" ||
            role === "troubleshoot_manager"
        );
    }, [currentUser]);
    const canSendReminderForTask = useCallback(
        (task: Task): boolean => {
            if (isAdminLike) return true;
            const myEmail = String((currentUser as any)?.email || "")
                .trim()
                .toLowerCase();
    ...
    const submitSendReminder = useCallback(
        async (message: string) => {
            const taskId = String(
                (sendReminderTask as any)?.id || (sendReminderTask as any)?._id || "",
            ).trim();
            if (!taskId) return;
            if (sendingReminderByTaskId[taskId]) return;
            setSendingReminderByTaskId((prev) => ({ ...prev, [taskId]: true }));
            try {
                const res = await apiClient.post("/reminders/send", {
                    taskId,
                    message,
                });
                if (res?.data?.success) {
                ...
                toast.error(
                    e?.response?.data?.message || e?.message || "Failed to send reminder",
                );
            } finally {
    ...
    const usersForAddTaskModal = useMemo(() => {
        ...
        const userCompanyKey = normalizeCompany(
            (currentUser as any)?.companyName || (currentUser as any)?.company,
        );
        ...
        const filterByCompany = (
            ...
            if (!targetCompanyKey) {
                ...
                return source.filter(
                    (u: any) =>
                        normalizeCompany((u as any)?.companyName || (u as any)?.company) ===
                        userCompanyKey,
                );
            }
            return source.filter((u: any) => {
                const cKey = normalizeCompany(
                    (u as any)?.companyName || (u as any)?.company,
                );
        ...
        const myEmailForAddTaskModal = String((currentUser as any)?.email || "")
            .trim()
            .toLowerCase();
        ...
        const taskTypeKey = (newTask.taskType || "")
            .toString()
            .trim()
            .toLowerCase();
        ...
        if (role === "md_manager") {
            ...
            const mdManagers = baseUsers.filter(
                (u: any) => normalizeRole(u?.role) === "md_manager",
            );
            const managersAndObManagersAndAssistants = filterByCompany(
            ...
            const candidates = [
                ...mdManagers,
                selfUser,
                ...managersAndObManagersAndAssistants,
            ];
            return Array.from(
                new Map(
                    candidates.map((u: any) => [toId(u) || String(u?.email || ""), u]),
                ).values(),
            );
        }
        ...
        if (role === "sbm" || role === "rm" || role === "am" || role === "ar") {
            const requesterId = toId(currentUser);
            const requesterManagerId = String(
                (currentUser as any)?.managerId || "",
            ).trim();
            ...
            if (role === "rm") {
                ...
                const sbmUser = baseUsers.filter((u: any) => {
                    const r = normalizeRole(u?.role);
                    return (
                        r === "sbm" && requesterManagerId && toId(u) === requesterManagerId
                    );
                });
                ...
                return Array.from(
                    new Map(
                        visible.map((u: any) => [toId(u) || String(u?.email || ""), u]),
                    ).values(),
                );
            }
            if (role === "am") {
                ...
                const rmUser = baseUsers.filter((u: any) => {
                    const r = normalizeRole(u?.role);
                    return (
                        r === "rm" && requesterManagerId && toId(u) === requesterManagerId
                    );
                });
            ...
            if (role === "sbm") {
                ...
                const byRole = baseUsers.filter((u: any) => {
                    const r = normalizeRole(u?.role);
                    return (
                        r === "sbm" || r === "rm" || r === "am" || r === "sales_manager"
                    );
                });
                const scoped = filterByCompany(byRole);
        ...
        return Array.from(
            new Map(
                merged.map((u: any) => [toId(u) || String((u as any)?.email || ""), u]),
            ).values(),
        );
    ...
    const handleLogout = useCallback(() => {
        try {
            const accessToken = sessionStorage.getItem(
                "tms_google_calendar_access_token",
            );
    ...
    const loadUsersForCompany = useCallback(async (companyName: string) => {
        ...
        try {
            ...
            if (res?.success && Array.isArray(res.data))
                setCompanyUsers(res.data as any);
    ...
    const [taskTypeIdsByBrandId, setTaskTypeIdsByBrandId] = useState<
        Record<string, string[]>
    >({});
    const [
        taskTypeIdsByCompanyUserBrandKey,
        setTaskTypeIdsByCompanyUserBrandKey,
    ] = useState<Record<string, string[]>>({});
    const [brandNamesByCompanyUserKey, setBrandNamesByCompanyUserKey] = useState<
        Record<string, string[]>
    >({});
    ...
    const normalizeCompanyKey = useCallback(
    ...
    const SPEED_E_COM_FIXED_TASK_TYPES = [
        "Meeting Pending",
        "CP Pending",
        "Recharge Negative",
    ];
    ...
    const isMdImpexUser = useMemo(() => {
        ...
        return key === "mdimpex" || key === "md_impex";
    }, [currentUser, normalizeCompanyKey]);
    const isSpeedEcomTask = useCallback(
        (task: any): boolean => {
    ...
    const getTaskAssigneeEmail = useCallback((task: any): string => {
        ...
        const email =
            (typeof assignedTo === "string" && assignedTo.includes("@")
                ? assignedTo
                : assignedTo?.email) ||
            (typeof assignedToUser === "string" && assignedToUser.includes("@")
                ? assignedToUser
                : assignedToUser?.email) ||
    ...
    const restrictTaskTypesForCompany = useCallback(
        (companyName: unknown, list: string[]): string[] => {
            ...
            const currentUserCompanyKey = normalizeCompanyKey(
                (currentUser as any)?.companyName || (currentUser as any)?.company,
            );
            ...
            if (
                companyKey === "all" &&
                currentUserCompanyKey === SPEED_E_COM_COMPANY_KEY
            ) {
                const safe = Array.isArray(list) ? list.filter(Boolean) : [];
                return safe.length > 0 ? safe : [...SPEED_E_COM_FIXED_TASK_TYPES];
            }
    ...
    const [taskTypeCompanyOverrides, setTaskTypeCompanyOverrides] = useState<
        Record<string, string[]>
    >({});
    ...
    const addTaskTypesToCompany = useCallback(
        async (companyName: string, typeNames: string[]) => {
            ...
            try {
                ...
                setTaskTypeCompanyOverrides((prev) => {
                    const current = prev || {};
                    const existing = Array.isArray(current[companyKey])
                        ? current[companyKey]
                        : [];
    ...
    const availableCompanies = useMemo(() => {
        ...
        if (needsCompanyList && fromCompanies.length > 0) {
            ...
            if (
                isSalesRole &&
                !list.some((c) => normalizeCompanyKey(c) === SPEED_E_COM_COMPANY_KEY)
            ) {
        ...
        const uniqueCompanies = [
            ...new Set(brands.map((brand: any) => getBrandCompanyNameSafe(brand))),
        ];
    ...
    const availableRmUsersForFilters = useMemo(() => {
        ...
        const fallbackCompanyKey = normalizeCompanyKey(
            (currentUser as any)?.companyName || (currentUser as any)?.company,
        );
        const companyKeyForRms =
            selectedCompanyKey && selectedCompanyKey !== "all"
                ? selectedCompanyKey
                : fallbackCompanyKey;
        const list: any[] = Array.isArray(usersRef.current)
            ? (usersRef.current as any[])
            : (users as any[]);
        ...
        const companyMatched = companyKeyForRms
            ? allRms.filter(
                (u: any) =>
                    Boolean(u?.companyKey) && u.companyKey === companyKeyForRms,
            )
            : [];
    ...
    useEffect(() => {
        ...
        const fromList =
            role === "md_manager" || role === "ob_manager"
        ...
        const defaultCompany =
            resolvedCompany ||
            (["sales_manager", "sales_man"].includes(role)
                ? SPEED_E_COM_COMPANY_NAME
                : MD_IMPEX_COMPANY_NAME);
        const allowedSet = new Set(
            (availableCompanies || []).map((c) => (c || "").toString().trim()),
        );
        setNewTask((prev) => {
    ...
    const allowedTaskTypeKeysForManager = useMemo(() => {
        ...
        const getAssignerRole = (t: any): string => {
            ...
            const direct = (assignedByUser?.role || assignedBy?.role || "")
                .toString()
                .trim()
                .toLowerCase();
            ...
            const rawIdOrEmail =
                (typeof assignedBy === "string"
                    ? assignedBy
                    : assignedBy?._id || assignedBy?.id || assignedBy?.email) ||
        ...
        const isAssignedToMe = (t: any) => {
            ...
            const assignedToId =
                ...
                (typeof assignedToUser === "string"
                    ? assignedToUser
                    : assignedToUser?._id || assignedToUser?.id) ||
                "";
            const assignedToEmail =
                (typeof assignedTo === "string" && assignedTo.includes("@")
                    ? assignedTo
                ...
                (typeof assignedToUser === "string" && assignedToUser.includes("@")
                    ...
                    : assignedToUser?.email) ||
                "";
            if (myId && assignedToId && assignedToId.toString().trim() === myId)
                return true;
            if (
                myEmail &&
                assignedToEmail &&
                assignedToEmail.toString().trim().toLowerCase() === myEmail
            )
                return true;
            return false;
        };
        ...
        (tasks || []).forEach((t: any) => {
            ...
            if (assignerRole !== "md_manager" && assignerRole !== "ob_manager")
                return;
    ...
    const availableTaskTypes = useMemo(() => {
            ...
            const isMine = (t: any) => {
                if (role === "md_manager" || role === "ob_manager") {
                    ...
                    const assignedById =
                        (typeof assignedBy === "string"
                            ? assignedBy
                            : assignedBy?._id || assignedBy?.id) ||
                        (typeof assignedByUser === "string"
                            ? assignedByUser
                            : assignedByUser?._id || assignedByUser?.id) ||
                        "";
                    const assignedByEmail =
                        (typeof assignedBy === "string" && assignedBy.includes("@")
                            ? assignedBy
                            : assignedBy?.email) ||
                        (typeof assignedByUser === "string" && assignedByUser.includes("@")
                            ? assignedByUser
                            : assignedByUser?.email) ||
                        "";
                    if (myId && assignedToId && assignedToId.toString().trim() === myId)
                    ...
                    if (
                        ...
                        return true;
                    if (myId && assignedById && assignedById.toString().trim() === myId)
                        return true;
                    if (
                        myEmail &&
                        assignedByEmail &&
                        assignedByEmail.toString().trim().toLowerCase() === myEmail
                    )
                        return true;
                    return false;
                }
        ...
        if (role === "manager") {
            ...
            const allowedKeys = new Set<string>(
                managerDefaultTypeLabels.map((t) => t.toLowerCase()),
            );
    ...
    const getTaskTypesForCompany = useCallback(
        (companyName: string): string[] => {
            const companyKey = normalizeText(companyName);
            if (!companyKey) return [];
            const fromTasks = Array.from(
                taskTypesByCompanyFromTasks.get(companyKey) || [],
            );
            const fromOverrides = Array.isArray(
            ...
            if (selectedCompanyKey === SPEED_E_COM_COMPANY_KEY) {
                const speedCompanyId =
                    (companies || []).find((c: any) => {
                        const name = String(
                            c?.name || c?.companyName || c?.title || "",
                        ).trim();
                ...
                return restrictTaskTypesForCompany(companyName, combined).sort((a, b) =>
                    a.localeCompare(b),
                );
            }
            const role = (currentUser?.role || "").toString().toLowerCase();
            if (role === "manager") {
                ...
                return merged
                    .filter((t) =>
                        allowedKeys.has((t || "").toString().trim().toLowerCase()),
                    )
        ...
        [
            allowedTaskTypeKeysForManager,
            companies,
            currentUser?.role,
            normalizeCompanyKey,
            normalizeText,
            restrictTaskTypesForCompany,
            taskTypeCompanyOverrides,
            taskTypes,
            taskTypesByCompanyFromTasks,
        ],
    );
    ...
    const getTaskTypesForCompanyBrand = useCallback(
        (companyName: string, brandName: string): string[] => {
            const companyKey = normalizeCompanyKey(companyName);
            ...
            const brandDoc: any = (apiBrands || []).find((b: any) => {
                ...
                return (
                    normalizeCompanyKey(bCompany) === companyKey &&
                    normalizeText(bName) === normalizeText(brandKey)
                );
            });
        ...
        [
            ...
            normalizeCompanyKey,
            normalizeText,
            taskTypeIdsByBrandId,
            taskTypes,
        ],
    );
    const getTaskTypesForCompanyUser = useCallback(
        (companyName: string, assignedToEmail: string): string[] => {
            const companyKey = normalizeCompanyKey(companyName);
            const emailKey = stripDeletedEmailSuffix(assignedToEmail)
                .trim()
                .toLowerCase();
            ...
            if (!userId) {
                const myEmailKey = stripDeletedEmailSuffix(currentUser?.email || "")
                    .trim()
                    .toLowerCase();
            ...
            const ids: string[] = [];
            Object.entries(taskTypeIdsByCompanyUserBrandKey || {}).forEach(
                ([key, value]) => {
    ...
    const getTaskTypesForCompanyUserBrand = useCallback(
        (
            companyName: string,
            brandName: string,
            ...
            const brandDoc: any = (apiBrands || []).find((b: any) => {
                ...
                const bName = getBrandNameSafe(b);
                return (
                    normalizeText(bCompany) === companyKey &&
                    normalizeText(bName) === normalizeText(brandKey)
                );
            });
            const brandId = (brandDoc?.id || brandDoc?._id || "").toString();
    ...
    const assistantManagerEmail = useMemo(() => {
        ...
        return stripDeletedEmailSuffix(manager?.email || "")
            .trim()
            .toLowerCase();
    ...
    const assistantScopedTasks = useMemo(() => {
        ...
        const myEmail = stripDeletedEmailSuffix(currentUser?.email || "")
            .trim()
            .toLowerCase();
        ...
        return (tasks || []).filter((t: any) => {
            const assignedToEmail = stripDeletedEmailSuffix(
                (t as any)?.assignedTo || "",
            )
                .trim()
                .toLowerCase();
    ...
    const availableBrands = useMemo(() => {
        ...
        if (role === "assistant") {
            ...
            const taskBrands = (assistantScopedTasks || [])
                .filter((t: any) => {
                    ...
                    const taskCompany = normalizeCompanyKey(
                        (t as any)?.companyName || (t as any)?.company,
                    );
        ...
        return brands
            .filter(
                (brand: any) =>
                    normalizeCompanyKey(getCompanyName(brand)) === companyKey,
            )
            .map((brand: any) => getBrandName(brand))
            .filter(Boolean)
            .sort();
    }, [
        assistantScopedTasks,
        brands,
        currentUser?.role,
        filters.company,
        normalizeCompanyKey,
    ]);
    const getBrandLabelForFilter = useCallback(
        (brandName: string): string => {
            const plain = String(brandName || "").trim();
            if (!plain) return "";
            const normalizedName = normalizeText(plain);
            const companyFilterKey = normalizeCompanyKey(
                filters.company === "all" ? "" : filters.company,
            );
            let candidates = (brands || []).filter(
                (b: any) => normalizeText(getBrandNameSafe(b)) === normalizedName,
            );
            if (companyFilterKey && companyFilterKey !== "all") {
                const byCompany = candidates.filter(
                    (b: any) =>
                        normalizeCompanyKey(getBrandCompanyNameSafe(b)) ===
                        companyFilterKey,
                );
    ...
    const availableTaskTypesForFilters = useMemo(() => {
        ...
        const ensureBrandAssignment = (list: string[]) => {
            ...
            const has = list.some(
                (x) => (x || "").toString().trim().toLowerCase() === "brand assignment",
            );
            if (has) return list;
            return [...list, label];
        };
        const roleKey = normalizeRoleKey(currentUser?.role);
        const isPersonScoped =
            roleKey === "assistant" ||
            roleKey === "sbm" ||
            roleKey === "rm" ||
            roleKey === "am" ||
        ...
        if (roleKey === "assistant") {
            const companyKey = normalizeText(
                filters.company === "all" ? "" : filters.company,
            );
            const brandKey = normalizeText(
                filters.brand === "all" ? "" : filters.brand,
            );
            const filtered = (assistantScopedTasks || [])
                .filter((t: any) => {
                    if (companyKey) {
                        const taskCompany = normalizeText(
                            (t as any)?.companyName || (t as any)?.company,
                        );
                ...
                .map((t: any) =>
                    String((t as any)?.taskType || (t as any)?.type || "").trim(),
                )
                .filter(Boolean);
            const merged = ensureBrandAssignment(Array.from(new Set(filtered)));
            return restrictTaskTypesForCompany(
                filters.company,
        ...
        if (filters.company !== "all" && filters.brand !== "all") {
            if (isPersonScoped) {
                return restrictTaskTypesForCompany(
                    filters.company,
                    ensureBrandAssignment(
                        getTaskTypesForCompanyUserBrand(
                            filters.company,
                            filters.brand,
                            currentUser?.email || "",
                        ),
            ...
            return restrictTaskTypesForCompany(
                filters.company,
                ensureBrandAssignment(
                    getTaskTypesForCompanyBrand(filters.company, filters.brand),
                ),
            );
        }
        if (filters.company !== "all") {
            ...
            return restrictTaskTypesForCompany(
                filters.company,
                ensureBrandAssignment(getTaskTypesForCompany(filters.company)),
            );
        }
        const fromOverrides = Object.values(taskTypeCompanyOverrides || {}).flatMap(
            (arr) => (Array.isArray(arr) ? arr : []),
        );
        const fromTasks = Array.from(taskTypesByCompanyFromTasks.values()).flatMap(
            (set) => Array.from(set),
        );
        const merged = ensureBrandAssignment(
            Array.from(
                new Set([...availableTaskTypes, ...fromOverrides, ...fromTasks]),
            ),
        );
        ...
        return restrictTaskTypesForCompany(
            filters.company,
            dedupeByCanonicalKey(merged).sort((a, b) => a.localeCompare(b)),
        );
    }, [
        allowedTaskTypeKeysForManager,
        assistantScopedTasks,
    ...
    const availableTaskTypesForNewTask = useMemo(() => {
        ...
        const baseCompany = () =>
            restrictTaskTypesForCompany(company, getTaskTypesForCompany(company));
        ...
        if (
            ...
            role === "super_admin" ||
            isSalesManager ||
            normalizeCompanyKey(company) === SPEED_E_COM_COMPANY_KEY
        ) {
        ...
        if (effectiveEmail && newTask.brand) {
            const specific = restrictTaskTypesForCompany(
                company,
                getTaskTypesForCompanyUserBrand(company, newTask.brand, effectiveEmail),
            );
            if (specific.length > 0) return ensureManagerOtherWork(specific);
            const brandLevel = restrictTaskTypesForCompany(
                company,
                getTaskTypesForCompanyBrand(company, newTask.brand),
            );
            if (brandLevel.length > 0) return ensureManagerOtherWork(brandLevel);
            return ensureManagerOtherWork(baseCompany());
        }
        if (effectiveEmail) {
            const userLevel = restrictTaskTypesForCompany(
                company,
                getTaskTypesForCompanyUser(company, effectiveEmail),
            );
    ...
    const availableTaskTypesForEditTask = useMemo(() => {
        ...
        if (
            ...
            role === "super_admin" ||
            isSalesManager ||
            normalizeCompanyKey(editFormData.companyName) === SPEED_E_COM_COMPANY_KEY
        ) {
            const baseCompany = restrictTaskTypesForCompany(
                editFormData.companyName,
                getTaskTypesForCompany(editFormData.companyName),
            );
            ...
            const exists = withOtherWork.some(
                (t) =>
                    (t || "").toString().trim().toLowerCase() === current.toLowerCase(),
            );
            if (exists) return withOtherWork;
            return [...withOtherWork, current];
        }
        if (editFormData.brand && editFormData.assignedTo) {
            const fromPerson = restrictTaskTypesForCompany(
                editFormData.companyName,
                getTaskTypesForCompanyUserBrand(
                    editFormData.companyName,
                    editFormData.brand,
        ...
        if (editFormData.assignedTo) {
            const fromUser = restrictTaskTypesForCompany(
                editFormData.companyName,
                getTaskTypesForCompanyUser(
                    editFormData.companyName,
                    editFormData.assignedTo,
    ...
    const fetchCompanyBrandTaskTypeMapping = useCallback(
        async (companyName: string, brandName: string) => {
            try {
                const company = (companyName || "").toString().trim();
                ...
                const brandDoc: any = (apiBrands || []).find((b: any) => {
                    ...
                    const bName = getBrandNameSafe(b);
                    return (
                        normalizeCompanyKey(bCompany) === normalizeCompanyKey(company) &&
                        normalizeText(bName) === normalizeText(brand)
                    );
                });
                const brandId = (brandDoc?.id || brandDoc?._id || "").toString();
                if (!brandId) return;
                const res = await companyBrandTaskTypeService.getMapping({
                    companyName: company,
                    brandId,
                    brandName: brand,
                });
    ...
    const fetchUserBrandTaskTypeMappings = useCallback(
        async (companyName: string, assignedToEmail: string) => {
            try {
                const company = (companyName || "").toString().trim();
                const email = stripDeletedEmailSuffix(assignedToEmail)
                    .trim()
                    .toLowerCase();
                ...
                if (!userId) return;
                const res = await assignService.getUserMappings({
                    companyName: company,
                    userId,
                });
                const next: Record<string, string[]> = {};
                ...
                (res?.data || []).forEach((m: any) => {
                    ...
                    if (!brandId) return;
                    const ids = Array.isArray(m?.taskTypeIds)
                        ? m.taskTypeIds
                ...
                setBrandNamesByCompanyUserKey((prev) => {
                    ...
                    const sorted = Array.from(brandNames)
                        .filter(Boolean)
                        .sort((a, b) => a.localeCompare(b));
    ...
    const fetchUserBrandTaskTypeMappingsCached = useCallback(
        async (companyName: string, assignedToEmail: string) => {
            const companyKey = normalizeCompanyKey(companyName);
            const emailKey = stripDeletedEmailSuffix(assignedToEmail)
                .trim()
    ...
    useEffect(() => {
        ...
        const email = stripDeletedEmailSuffix(currentUser?.email || "")
            .trim()
            .toLowerCase();
        if (!email) return;
        ...
        currentUser?.email,
        currentUser?.role,
        fetchUserBrandTaskTypeMappingsCached,
        filters.company,
    ]);
    useEffect(() => {
        const handler = (e: any) => {
            const detail = e?.detail || {};
            fetchCompanyBrandTaskTypeMapping(
                detail?.companyName || newTask.companyName,
                detail?.brandName || newTask.brand,
            );
        };
        window.addEventListener("companyBrandTaskTypesUpdated", handler as any);
        return () =>
            window.removeEventListener(
                "companyBrandTaskTypesUpdated",
                handler as any,
            );
    ...
    useEffect(() => {
        ...
        void fetchUserBrandTaskTypeMappingsCached(
            editFormData.companyName,
            editFormData.assignedTo,
        );
    }, [
    ...
    const handleSpeedEcomReassignSubmit = useCallback(
        async (payload: { assignedTo: string; dueDate: string }) => {
            ...
            try {
                ...
                const message =
                    e?.response?.data?.message || e?.message || "Failed to reassign task";
                toast.error(message);
                return false;
            } finally {
    ...
    const getTaskBorderColor = useCallback(
        (task: Task): string => {
    ...
    const canEditTask = useCallback(
        (task: Task): boolean => {
            ...
            const normalizeCompanyKey = (v: unknown): string =>
                String(v || "")
                    .trim()
                    .toLowerCase()
                    .replace(/\s+/g, "");
            const isMdImpexTask =
                normalizeCompanyKey(task?.companyName || (task as any)?.company) ===
                "mdimpex";

            // If it's an MD Impex role, strictly respect the access logic
            if (
                isMdImpexTask &&
                (role === "md_manager" || role === "ob_manager" || role === "troubleshoot_manager" || role === "manager")
            ) {
                return hasAccess("edit_any_task");
            }
    ...
    const canMarkTaskDone = useCallback(
        (task: Task) => {
            ...
            const myEmail = stripDeletedEmailSuffix(currentUser?.email)
                .trim()
                .toLowerCase();
            const assignedToEmail = stripDeletedEmailSuffix((task as any)?.assignedTo)
                .trim()
                .toLowerCase();
    ...
    const handleUpdateUser = useCallback(
        async (userId: string, updatedData: Partial<UserType>) => {
            ...
            if (!allowedByPerms) {
                const target = (usersRef.current || []).find(
                    (u: any) => (u?.id || u?._id || "").toString() === userId?.toString(),
                );
                const targetRole = (target?.role || "").toString().trim().toLowerCase();
                const toRoleKey = (v: unknown) =>
                    String(v || "")
                        .trim()
                        .toLowerCase()
                        .replace(/[\s-]+/g, "_");
                ...
                const isAssistantLike = (rk: string) =>
                    rk === "assistant" ||
                    rk === "assistance" ||
                    rk === "assistence" ||
                    rk === "assistece" ||
                ...
                if (requesterRoleKey === "md_manager") {
                    ...
                    // Allow MD Manager to edit users they created
                    const isCreatedByMe = targetCreatedByEmail && currentUserEmail && targetCreatedByEmail === currentUserEmail;
                    if (!isCreatedByMe && targetRoleKey !== "manager" && !isAssistantLike(targetRoleKey))
                        throw new Error("Only administrators can edit users");
                } else if (requesterRoleKey === "ob_manager") {
                    if (!isAssistantLike(targetRoleKey))
                        throw new Error("Only administrators can edit users");
                } else if (requesterRoleKey === "manager") {
                    if (!isAssistantLike(targetRoleKey))
                        throw new Error("Only administrators can edit users");
                } else if (requesterRoleKey === "sbm") {
                    if (targetRoleKey !== "rm" && targetRoleKey !== "am")
                        throw new Error("Only administrators can edit users");
    ...
    const handleCreateUser = useCallback(
        async (newUser: Partial<UserType>) => {
            const requesterRole = (currentUser?.role || "")
                .toString()
                .trim()
                .toLowerCase();
            ...
            if (!allowedByPerms) {
                ...
                if (requesterRole === "md_manager") {
                    ...
                    if (targetRole !== "assistant" && targetRole !== "sub_assistance")
                        throw new Error("You do not have permission to create users");
                } else if (requesterRole === "manager") {
                    if (targetRole !== "assistant")
                        throw new Error("You do not have permission to create users");
                } else if (requesterRole === "sbm") {
                    if (targetRole !== "rm" && targetRole !== "am")
                        throw new Error("You do not have permission to create users");
            ...
            try {
                ...
                const roleKey = (
                    isManager ? "assistant" : (newUser.role as any) || "assistant"
                ) as string;
                const payload = {
                    name: newUser.name || "",
                    email: newUser.email || "",
                    ...
                    role: roleKey,
                    managerId:
                        roleKey === "assistant" ? undefined : (newUser as any).managerId,
                    companyName:
        ...
        async (userId: string) => {
            ...
            if (!allowedByPerms) {
                ...
                if (requesterRole === "md_manager") {
                    const targetCreatedByEmail = (target as any)?.createdByEmail?.toString().trim().toLowerCase() || "";
                    const currentUserEmail = (currentUser as any)?.email?.toString().trim().toLowerCase() || "";
                    // Allow MD Manager to delete users they created
                    const isCreatedByMe = targetCreatedByEmail && currentUserEmail && targetCreatedByEmail === currentUserEmail;
                    if (
                        !isCreatedByMe &&
                        targetRole !== "manager" &&
            ...
            try {
                ...
                const isSuccess =
                    response && (response.success === true || !response.error);
                if (isSuccess) {
                    setUsers((prev) =>
                        prev.filter((user) => (user.id || (user as any)._id) !== userId),
                    );
                } else {
        ...
        (task: Task): { name: string; email: string } => {
            ...
            if (task.assignedTo) {
                ...
                if (typeof task.assignedTo === "object" && task.assignedTo !== null) {
                    const email = stripDeletedEmailSuffix(
    ...
    const availableBrandsForNewTask = useMemo((): Array<{
        value: string;
        label: string;
    }> => {
        const company = newTask.companyName;
        ...
        const addOption = (plainName: string) => {
            ...
            const brandDoc: any = (brands || []).find(
                (b: any) =>
        ...
        if (email) {
            ...
            const assigned = Array.isArray(brandNamesByCompanyUserKey[cuKey])
                ? brandNamesByCompanyUserKey[cuKey]
                : [];
        ...
        (brands || [])
            .filter(
                (brand) =>
                    normalizeCompanyKey(getBrandCompanyNameSafe(brand)) === companyKey,
            )
        ...
        brandNamesByCompanyUserKey,
        brands,
        currentUser?.email,
        getBrandCompanyNameSafe,
        getBrandNameSafe,
        newTask.companyName,
        normalizeCompanyKey,
        normalizeText,
    ...
    const getEditFormBrandOptions = useCallback((): Array<{
        value: string;
        label: string;
    }> => {
        ...
        const email = stripDeletedEmailSuffix(editFormData.assignedTo)
            .trim()
            .toLowerCase();
        if (email) {
            const userDoc: any = (usersRef.current || []).find((u: any) => {
    ...
    const formatBrandWithGroupNumber = useCallback(
        (task: any): string => {
            ...
            const byId = brandId
                ? (brands || []).find(
                    (b: any) => String(b?.id || b?._id || "").toString() === brandId,
                )
                : null;
    ...
    const handleSaveComment = useCallback(
        async (taskId: string, comment: string): Promise<CommentType> => {
            try {
    ...
    const handleFetchTaskComments = useCallback(
        async (taskId: string): Promise<CommentType[]> => {
            try {
    ...
    const getCommentSidebarComments = useCallback(
        (taskId: string): CommentType[] => {
            const key = String(taskId || "").trim();
            if (!key) return [];
            return Array.isArray(commentSidebarCommentsByTaskId[key])
                ? commentSidebarCommentsByTaskId[key]
                : [];
        },
        [commentSidebarCommentsByTaskId],
    );
    const handleOpenTaskCommentSidebar = useCallback(
        async (task: Task) => {
            if (!task?.id) return;
            if (!isSbmUser) return;
    ...
    const handleReassignTask = useCallback(
        async (taskId: string, newAssigneeId: string, dueDate?: string) => {
            try {
                ...
                const normalizeEmailSafe = (v: unknown): string =>
                    stripDeletedEmailSuffix(v).trim().toLowerCase();
                const myEmail = normalizeEmailSafe((currentUser as any)?.email);
                const myId = String(
                    (currentUser as any)?.id || (currentUser as any)?._id || "",
                ).trim();
                ...
                const isObManager =
                    myRoleKey === "ob_manager" || myRoleKey === "obmanager";
                const isManagerRole =
                    myRoleKey === "manager" || myRoleKey === "md_manager";
                const isSbmRole = myRoleKey === "sbm";
                const isRmOrAmRole = myRoleKey === "rm" || myRoleKey === "am";
                const assignedByEmail = normalizeEmailSafe(
                    (task as any)?.assignedByUser?.email || (task as any)?.assignedBy,
                );
                ...
                const isSpeedEcomTask =
                    normalizeCompanyKey(
                        (task as any)?.companyName || (task as any)?.company,
                    ) === "speedecom";
                const taskStatusKey = String((task as any)?.status || "")
                    .trim()
                    .toLowerCase();
                ...
                const resolveAssignerRole = (t: any): string => {
                    const direct = normalizeRole(
                        t?.assignedByUser?.role || t?.assignedBy?.role,
                    );
                    if (direct) return direct;
                ...
                const directEmail =
                    newAssigneeId && newAssigneeId.includes("@")
                        ? normalizeEmailSafe(newAssigneeId)
                        : "";
                const isRutuDirect = Boolean(
                    directEmail && directEmail === normalizeEmailSafe(RUTU_EMAIL),
                );
                const newAssignee = directEmail
                    ? findUserByIdOrEmail(directEmail)
                    : findUserByIdOrEmail(newAssigneeId);
                const nextAssigneeEmail =
                    directEmail || normalizeEmailSafe((newAssignee as any)?.email);
                const nextAssigneeRole = normalizeRole((newAssignee as any)?.role);
                const isSubAssistance = nextAssigneeRole === "sub_assistance";
                const isAssistantCandidate = Boolean(
                    nextAssigneeEmail &&
                    (isAssistantRole(nextAssigneeRole) ||
                        isAssistantRole((newAssignee as any)?.role)),
                );
                ...
                const assignedToCandidate: any =
                    (task as any)?.assignedToUser || (task as any)?.assignedTo;
                const assignedToId =
                    typeof assignedToCandidate === "object"
                        ? String(
                            assignedToCandidate?.id || assignedToCandidate?._id || "",
                        ).trim()
                        : "";
                const assignedToEmail = normalizeEmailSafe(
                    typeof assignedToCandidate === "string"
                        ? assignedToCandidate.includes("@")
                            ? assignedToCandidate
                            : ""
                ...
                const assignedByCandidateResolved: any =
                    (task as any)?.assignedByUser || (task as any)?.assignedBy;
                const assignedById =
                    typeof assignedByCandidateResolved === "object"
                        ? String(
                            assignedByCandidateResolved?.id ||
                            assignedByCandidateResolved?._id ||
                            "",
                        ).trim()
                        : "";
                const assignedByEmailKey = normalizeEmailSafe(
                    typeof assignedByCandidateResolved === "string"
                        ? assignedByCandidateResolved.includes("@")
                            ? assignedByCandidateResolved
                            : ""
                        : assignedByCandidateResolved?.email || "",
                );
                const assignedByUser =
                    assignedById || assignedByEmailKey
                        ? findUserByIdOrEmail(assignedById || assignedByEmailKey)
                        : undefined;
                const assignedByUserId = String(
                    (assignedByUser as any)?.id ||
                    (assignedByUser as any)?._id ||
                    assignedById ||
                    "",
                ).trim();
                const canReassignByPairEmailFallback = (() => {
                    if (!isRmOrAmRole || !isTaskCompleted) return false;
                    const creatorEmailKey = normalizeEmailSafe(
                        assignedByEmailKey || assignedByEmail,
                    );
                    ...
                    if (myRoleKey === "am") {
                        ...
                        const myManager = myManagerIdKey
                            ? list.find(
                                (u: any) =>
                                    String(u?.id || u?._id || "").trim() === myManagerIdKey,
                            )
                            : undefined;
                    ...
                    ? Boolean(
                        ...
                        (isTaskAssigner ||
                            ...
                            (isRmOrAmRole &&
                                (isTaskAssignee ||
                                    (assignedByUserId &&
                                        allowedPairUserIds.has(assignedByUserId))))),
                    )
                    : Boolean(
                        (isManagerRole && isTaskAssigner && nextAssigneeEmail) ||
                        (myEmail &&
                            myEmail === KEYURI_EMAIL &&
                            nextAssigneeEmail &&
                            (isRutuDirect || isSubAssistance)) ||
                        (isObManager &&
                            nextAssigneeEmail &&
                            (isAssistantCandidate || isObManagerDirectEmail)) ||
                ...
                if (response.success && response.data) {
                    dispatch(taskUpserted(response.data as Task));
                    toast.success(
                        `Task reassigned to ${newAssignee ? (newAssignee as any).name : nextAssigneeEmail}`,
                    );
                } else {
                    toast.error(response.message || "Failed to reassign task");
                }
    ...
    const handleMdImpexReassignTask = useCallback(
        async (taskId: string, newAssigneeEmail: string) => {
            try {
                const response = await taskService.mdImpexReassignTask(
                    taskId,
                    newAssigneeEmail,
                );
                if (response.success && response.data) {
                ...
                if (response.data) {
                    ...
                    dispatch(
                        taskUpserted({
                            ...
                            history: [
                                ...((tasks.find((t) => t.id === taskId) as any)?.history || []),
                                normalized,
                            ],
                        } as Task),
                    );
        ...
        async (taskId: string) => {
            try {
                ...
                if (response.success) {
                    dispatch(taskUpserted(updatedTask as Task));
                    toast.success(
                        task.completedApproval ? "Approval removed" : "Task approved",
                    );
                } else {
    ...
    const handleUpdateTaskApproval = useCallback(
        async (taskId: string, completedApproval: boolean) => {
            try {
                const task = tasks.find((t) => t.id === taskId);
                if (!task) {
                    toast.error("Task not found");
    ...
    const handleFetchTaskHistory = useCallback(
        async (taskId: string): Promise<TaskHistory[]> => {
            try {
    ...
    const getFilteredTasksByStat = useCallback(() => {
        ...
        const role = String((currentUser as any)?.role || "")
            ...
            .toLowerCase();
        const myEmail = (currentUser.email || "").toString().trim().toLowerCase();
        const normalizeTaskTypeKey = (t: any) =>
            String(t?.taskType || t?.type || "")
                .trim()
                .toLowerCase();
        const isOtherWorkTask = (t: any) =>
            normalizeTaskTypeKey(t) === "other work";
        const normalizeRoleKey = (v: unknown) =>
            String(v || "")
                .trim()
                .toLowerCase()
                .replace(/[\s-]+/g, "_");
        ...
        const isAssignedToMe = (t: any): boolean => {
            const email = String(
                (t as any)?.assignedToUser?.email || (t as any)?.assignedTo || "",
            )
                .trim()
                .toLowerCase();
            ...
            if (role === "ob_manager") {
                const assignedByMe =
                    String(
                        ...
                        .toLowerCase() === myEmail;
                return (
                    assignedByMe || isAssignedToMe(task) || isAssistantAssignee(task)
                );
            }
            if (role === "manager") {
        ...
        if (normalizeRoleKey(role) === "sbm") {
            const selectedRm = String((filters as any)?.rm || "")
                .trim()
                .toLowerCase();
            if (selectedRm && selectedRm !== "all") {
                ...
                const list: any[] = Array.isArray(usersRef.current)
                    ? (usersRef.current as any[])
                    : (users as any[]);
                const selectedRmDoc: any = (list || []).find(
                    (u: any) =>
                        String(u?.email || "")
                            .trim()
                            .toLowerCase() === selectedRm,
                );
                const selectedRmId = String(
                    selectedRmDoc?.id || selectedRmDoc?._id || "",
                ).trim();
                const teamEmails = rmTeamRaw
                    ...
                    : selectedRmId
                        ? (list || [])
                            .filter(
                                (u: any) =>
                                    String(u?.managerId || "").trim() === selectedRmId,
                            )
                            .filter((u: any) => {
        ...
        if (selectedStatFilter === "completed") {
            ...
            filtered = filtered.filter(
                (task) =>
                    task.status !== "completed" && isOverdue(task.dueDate, task.status),
            );
        }
        ...
        if (filters.taskType !== "all") {
            ...
            filtered = filtered.filter((task) => {
                const taskTypeKey = canonicalizeTypeKey(
                    task.taskType || (task as any).type || "",
                );
        ...
        if (filters.assigned) {
            ...
            if (assignedFilterValue === "assigned-to-me") {
                ...
                filtered = filtered.filter(
                    (task) => task.assignedTo === assignedToEmail,
                );
            }
        }
        if (searchTerm) {
            ...
            filtered = filtered.filter((task) => {
                ...
                const assignedToEmail = String(
                    ...
                    (typeof (task as any).assignedTo === "string"
                        ...
                        : (task as any).assignedTo?.email) ||
                    "",
                ).toLowerCase();
                const assignedToName = String(assignedToUser?.name || "").toLowerCase();
                const assignedByEmail = String(
                    assignedByUser?.email ||
                    (typeof (task as any).assignedBy === "string"
                        ? (task as any).assignedBy
                        : (task as any).assignedBy?.email) ||
                    "",
                ).toLowerCase();
                const assignedByName = String(assignedByUser?.name || "").toLowerCase();
                return (
                    title.includes(term) ||
    ...
    const showListActionsColumn = useMemo(() => {
        return displayTasks.some(
            (t: Task) => canEditTask(t) || canEditDeleteTask(t),
        );
    ...
    useMemo(() => {
        ...
        const isOtherWorkTask = (t: any) =>
            normalizeTaskTypeKey(t) === "other work";
        const resolveAssignerRole = (t: any) =>
            String(
                (t as any)?.assignedByUser?.role || (t as any)?.assilgnedBy?.role || "",
            )
                .trim()
                .toLowerCase();
        const normalizeRoleKey = (v: unknown) =>
        ...
        let filtered = tasks.filter((task) => {
            if (role === "ob_manager") {
                ...
                const resolveAssigneeRoleKey = (t: any): string => {
                    ...
                    if (direct) return direct;
                    const candidate =
                        (t as any)?.assignedToUser || (t as any)?.assignedTo;
                    const idOrEmail =
                        typeof candidate === "string"
                            ? candidate
                            : candidate?.id || candidate?._id || candidate?.email || "";
            ...
            if (role === "manager") {
                ...
                const resolveAssignerRole = (t: any) =>
                    String(
                        (t as any)?.assignedByUser?.role ||
                        (t as any)?.assignedBy?.role ||
                        "",
                    )
                        .trim()
                        .toLowerCase();
        ...
        if (normalizeRoleKey(role) === "sbm") {
            ...
            if (selectedRm && selectedRm !== "all") {
                ...
                const teamEmails = selectedRmId
                    ? (list || [])
                        .filter(
                            (u: any) => String(u?.managerId || "").trim() === selectedRmId,
                        )
                        .filter((u: any) => {
                            const r = normalizeRoleKey(u?.role);
                            return r === "am" || r === "ar";
                ...
                filtered = filtered.filter((t: any) => {
                    ...
                    const isToMe = Boolean(
                        myEmail && assignedToEmail && assignedToEmail === myEmail,
                    );
                    const isToTeam = Boolean(
                        assignedToEmail && teamEmails.includes(assignedToEmail),
                    );
    ...
    const setStatusFilter = useCallback(
        (v: string) => handleFilterChange("status", v),
        [handleFilterChange],
    );
    const setDateFilterInternal = useCallback(
        (v: string) => handleFilterChange("date", v),
        [handleFilterChange],
    );
    const setAssignedFilter = useCallback(
        (v: string) => handleFilterChange("assigned", v),
        [handleFilterChange],
    );
    ...
    const handleInputChange = useCallback(
        (field: keyof NewTaskForm, value: string) => {
            setNewTask((prev) => {
                const next: any = {
                    ...prev,
                    [field]: value,
                ...
                if (field === "assignedTo") {
                    ...
                    const userCompany = (userDoc?.companyName || userDoc?.company || "")
                        .toString()
                        .trim();
                    if (
                        userCompany &&
                        userCompany !== (next.companyName || "").toString().trim()
                    ) {
    ...
    useEffect(() => {
        ...
        if (!company) return;
        const options = Array.isArray(availableTaskTypesForNewTask)
    ...
    const handleAddTaskTypeClick = useCallback(async () => {
        ...
        if (!name) return;
        try {
            const res = await taskTypeService.createTaskType({
                name,
                ...
                setTaskTypes((prev) => {
                    ...
                    const exists = list.some(
                        (t) =>
                            ...
                            (res.data.name || "").toString().trim().toLowerCase(),
                    );
    ...
    const handleSubmitBulkBrands = useCallback(async () => {
        ...
        const requestedBrands = isSpeedEcomCompany
            ? (() => {
                const groupNumbers = trimEndEmpty(
                    splitLines((bulkBrandForm.groupNumber || "") as string),
                );
                const brandNames = trimEndEmpty(
                    splitLines((bulkBrandForm.groupName || "") as string),
                );
                ...
                for (let i = 0; i < brandNames.length; i += 1) {
                    ...
                    if (!groupNumber || !brandName) {
                        toast.error(
                            `Row ${i + 1}: Group Number and Brand Name are required`,
                        );
                        return [];
                    }
        ...
        try {
            ...
            for (let i = 0; i < batches.length; i += 1) {
                ...
                if (meta) {
                    assignmentMetaAgg.rmAmEmailCount = Math.max(
                        assignmentMetaAgg.rmAmEmailCount,
                        Number(meta.rmAmEmailCount || 0),
                    );
                    assignmentMetaAgg.rmAmUsersFound = Math.max(
                        assignmentMetaAgg.rmAmUsersFound,
                        Number(meta.rmAmUsersFound || 0),
                    );
                    assignmentMetaAgg.assignedBrandIdsOps += Number(
                        meta.assignedBrandIdsOps || 0,
                    );
            ...
            const res = {
                success: true,
                data: createdBrandsAll,
                meta: { assignment: assignmentMetaAgg },
            } as any;
            if (res.success && Array.isArray(res.data) && res.data.length > 0) {
                if (
                    isSpeedEcomCompany &&
                    (bulkBrandForm.rmEmail || bulkBrandForm.amEmail)
                ) {
                    const backendDidAssignment = Boolean(
                        (res as any)?.meta?.assignment?.mappingOps,
                    );
                ...
                const event = new CustomEvent("brandUpdated", {
                    detail: { brands: res.data },
                });
                window.dispatchEvent(event);
    ...
    const handleSubmitBulkCompanies = useCallback(async () => {
        ...
        try {
            ...
            if (res.success && res.data) {
                ...
                toast.success(
                    `${res.data.length} company(ies) processed successfully!`,
                );
            } else {
    ...
    const handleSubmitBulkTaskTypes = useCallback(async () => {
        ...
        try {
            ...
            if (res.success && res.data) {
                ...
                toast.success(
                    `${res.data.length} task type(s) processed successfully!`,
                );
            } else {
    ...
    const handleEditInputChange = useCallback(
        (field: keyof EditTaskForm, value: string) => {
    ...
    const validateForm = useCallback(
        (data?: NewTaskForm) => {
            ...
            if (!String(formToValidate.title || '').trim()) {
            ...
            // Company validation
            if (!String(formToValidate.companyName || '').trim()) {
                errors.companyName = "Company is required";
            }
            // Brand validation
            if (!String(formToValidate.brand || '').trim()) {
        ...
        async (options?: { force?: boolean }) => {
            try {
                const isInitialFetch =
                    !hasFetchedTasksOnceRef.current && tasks.length === 0;
                ...
                if (Array.isArray(incoming)) {
                    ...
                    const readSeenFromStorage = (email: string): Set<string> => {
                        try {
                            ...
                            return new Set(
                                parsed.map((x) => (x == null ? "" : String(x))).filter(Boolean),
                            );
                        } catch {
                            return new Set();
                        }
                    ...
                    const previousSeenIds = myEmail
                        ? readSeenFromStorage(myEmail)
                        : new Set<string>();
                    const canDetectNewTasks =
                        Boolean(myEmail) && previousSeenIds.size > 0;
                        ...
                        const getAssignerName = (task: Task): string => {
                            ...
                            if (typeof assignedBy === "string" && assignedBy) {
                                const match = (usersRef.current || []).find(
                                    (u) =>
                                        (u?.email || "").toLowerCase() === assignedBy.toLowerCase(),
                                );
    ...
    const fetchBrands = useCallback(async () => {
        try {
            ...
            const isSalesManager = role === "sales_manager" || role === "sales_man";
            const response =
                isAdminLike || isSalesManager
                    ? await brandService.getBrands({ includeDeleted: true, limit: 5000 })
                    : await brandService.getAssignedBrands({ limit: 5000 });
    ...
    const ensureUsersLoaded = useCallback(
        async (opts?: { force?: boolean }) => {
            ...
            const isFresh =
                usersFetchedAtRef.current &&
                Date.now() - usersFetchedAtRef.current < SUPPORTING_FETCH_TTL_MS;
    ...
    const ensureBrandsLoaded = useCallback(
        async (opts?: { force?: boolean }) => {
            const force = Boolean(opts?.force);
            const isFresh =
                brandsFetchedAtRef.current &&
                Date.now() - brandsFetchedAtRef.current < SUPPORTING_FETCH_TTL_MS;
    ...
    const ensureCompaniesLoaded = useCallback(
        async (opts?: { force?: boolean }) => {
            const force = Boolean(opts?.force);
            const isFresh =
                companiesFetchedAtRef.current &&
                Date.now() - companiesFetchedAtRef.current < SUPPORTING_FETCH_TTL_MS;
            if (!force && companies.length > 0 && isFresh) return;
            if (companiesFetchInFlightRef.current)
    ...
    const ensureTaskTypesLoaded = useCallback(
        async (opts?: { force?: boolean }) => {
            const force = Boolean(opts?.force);
            const isFresh =
                taskTypesFetchedAtRef.current &&
                Date.now() - taskTypesFetchedAtRef.current < SUPPORTING_FETCH_TTL_MS;
            if (!force && taskTypes.length > 0 && isFresh) return;
            if (taskTypesFetchInFlightRef.current)
    ...
    useEffect(() => {
        const handler = (e: any) => {
            ...
            void (async () => {
                ...
                const email = stripDeletedEmailSuffix(userDoc?.email)
                    .trim()
                    .toLowerCase();
                if (email)
                    await fetchUserBrandTaskTypeMappingsCached(companyName, email);
            })();
        };
        window.addEventListener("assignmentsApplied", handler as any);
        return () =>
            window.removeEventListener("assignmentsApplied", handler as any);
    }, [
        ensureBrandsLoaded,
    ...
    useEffect(() => {
        ...
        if (role === "sbm") {
            const defaultCompany =
                (availableCompaniesForSbm[0] || SPEED_E_COM_COMPANY_NAME || "")
                    .toString()
                    .trim() || SPEED_E_COM_COMPANY_NAME;
            setNewTask((prev) => {
                const current = (prev?.companyName || "").toString().trim();
                if (current) return prev;
                return { ...prev, companyName: defaultCompany };
    ...
    const openAddTaskModal = useCallback(() => {
        ...
        if (isMdImpexUser || role === "troubleshoot_manager") {
            setShowMdImpexAddTaskModal(true);
        } else {
    ...
    const handleOpenEditModal = useCallback(
        (task: Task) => {
            ...
            const isMdImpexTask =
                (task.companyName || "").toLowerCase().replace(/\s+/g, "") ===
                "mdimpex";
            if (isMdImpexTask) {
    ...
    const handleSaveTaskFromModal = useCallback(
        async (modalData?: NewTaskForm) => {
            const taskToUse = modalData || newTask;
            if (!validateForm(taskToUse)) {
                console.log("[DashboardPage] Validation failed for task data:", taskToUse);
                return;
            }
            ...
            try {
                const selectedBrandObj = brands.find(
                    (b) =>
                        ...
                        normalizeText((b as any).company || (b as any).companyName) ===
                        normalizeText(taskToUse.companyName),
                );
                const resolvedBrandId = (() => {
                    const candidate =
                        (selectedBrandObj as any)?.id || (selectedBrandObj as any)?._id;
                ...
                const taskData = {
                    ...
                    priority:
                        taskToUse.priority === "urgent" ? "high" : taskToUse.priority,
                ...
                if (response.success && response.data) {
                    ...
                    setShowMdImpexAddTaskModal(false);
                    setNewTask({
                        title: "",
                        assignedTo: "",
                        dueDate: "",
    ...
    const handleSaveEditedTask = useCallback(async () => {
        ...
        const resolvedTaskId = (
            (editFormData as any)?.id ||
            (editingTask as any)?.id ||
            (editingTask as any)?._id ||
            ""
        ).toString();
        if (!resolvedTaskId) {
        ...
        try {
            const myEmail = stripDeletedEmailSuffix(currentUser?.email || "")
                .trim()
                .toLowerCase();
            const previousDueDate = editingTask?.dueDate
                ? new Date(editingTask.dueDate).toISOString().split("T")[0]
                : "";
            const dueDateChanged = Boolean(
                previousDueDate &&
                editFormData.dueDate &&
                previousDueDate !== editFormData.dueDate,
            );
            const speedEcomTask = isSpeedEcomTask(editingTask);
            const isMdImpexTask =
                (
                    (editingTask as any)?.companyName ||
                    (editingTask as any)?.company ||
                    ""
                )
                    .toString()
                    .toLowerCase()
            ...
            const canEditDueDateForSpeedEcom =
                !speedEcomTask ||
                (myEmail && assigneeEmail && myEmail === assigneeEmail);
            const selectedBrandObj = brands.find(
                (b) =>
                    normalizeText(b.name) === normalizeText(editFormData.brand) &&
                    normalizeText((b as any).company || (b as any).companyName) ===
                    normalizeText(editFormData.companyName),
            );
            const resolvedBrandId = (() => {
                const candidate =
                    (selectedBrandObj as any)?.id || (selectedBrandObj as any)?._id;
            ...
            if (!canEditDueDateForSpeedEcom) {
                if (dueDateChanged) {
                    toast.error(
                        "Only the assignee can update due date for Speed E Com tasks",
                    );
    ...
    const handleToggleTaskStatus = useCallback(
        async (
            ...
            if (task.completedApproval && currentStatus === "completed") {
                toast.error(
                    "This task has been permanently approved and cannot be changed",
                );
                return;
            }
    ...
    const handleManagerCreateBrand = useCallback(async () => {
        ...
        const hasDuplicate = existingBrands.some((b: any) => {
            const bCompany = normalizeText(
                (b?.company || b?.companyName || "") as string,
            );
            if (bCompany !== normalizedCompany) return false;
            const bName = normalizeText(
                (b?.name || b?.brandName || b?.brand || "") as string,
            );
            return bName === normalizedName;
        });
        if (hasDuplicate) {
            const confirmed = window.confirm(
                `Brand "${name}" already exists for company "${company}". Do you still want to use it?`,
            );
            if (!confirmed) {
                return;
            }
                ...
                setApiBrands((prev) => {
                    ...
                    const exists = list.some(
                        (b) =>
                            ...
                            (res.data.name || "").toString().trim().toLowerCase() &&
                            (b?.company || "").toString().trim().toLowerCase() ===
                            (res.data.company || "").toString().trim().toLowerCase(),
                    );
                    if (exists) return list;
                    return [...list, res.data];
                });
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex">
            <SendReminderModal
                ...
                isSending={Boolean(
                    ...
                    String(
                        (sendReminderTask as any)?.id ||
                        (sendReminderTask as any)?._id ||
                        "",
                    )
                    ],
                )}
                        ...
                        <div className="px-6 py-5 space-y-4">
                            <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                                <div className="text-sm font-semibold text-gray-900 truncate">
                                    {String((reviewModalTask as any)?.title || "Task")}
                                </div>
                                <div className="mt-1 text-xs text-gray-600">
                                    {String(
                                        (reviewModalTask as any)?.assignedToUser?.name ||
                                        (reviewModalTask as any)?.assignedToUser?.email ||
                                        (reviewModalTask as any)?.assignedTo ||
                                        "",
                                    ).trim()
                        ...
                        <div className={dashboardContainerClasses}>
                            {currentView === "dashboard" ? (
                                <>
                                                        ...
                                                        <div className="border-t pt-4">
                                                            <div className="flex items-center justify-between mb-3">
                                                                ...
                                                                <span className="text-xs text-gray-500">
                                                                    {
                                                                        getCommentSidebarComments(
                                                                            String(commentSidebarTask.id),
                                                                        ).length
                                                                    }{" "}
                                                                    total
                                                                </span>
                                                    ...
                                                    <div className="flex items-start justify-between">
                                                        <div className="flex-1">
                                                            <div className="flex items-center gap-3 mb-3">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div className="flex items-baseline gap-2">
                                                                        ...
                                                                        <div
                                                                            className={`flex items-center text-xs font-medium px-2 py-1 rounded-full ${stat.changeType === "positive"
                                                            ...
                                                            <div className="flex items-center justify-between">
                                                                ...
                                                                <span
                                                                    className={`text-xs font-medium px-2 py-1 rounded-full ${selectedStatFilter === stat.id
                                    ...
                                    {!isSpeedEcomUser ? (
                                        String((currentUser as any)?.role || "")
                                            .trim()
                                            .toLowerCase() !== "troubleshoot_manager" ? (
                                            <>
                                                    ...
                                                    {(() => {
                                                        ...
                                                        return (
                                                            <button
                                                                type="button"
                                                                onClick={() =>
                                                                    setDashboardSpotlight("employee-of-month")
                                                                }
                                                                ...
                                                                <div className="flex items-start justify-between">
                                                                    <div>
                                                                        ...
                                                                        <p className="text-xs text-gray-500 mt-1 ">
                                                                            Based on manager reviews (month wise)
                                                                        </p>
                                                                    </div>
                                                                    <span
                                                                        className={`text-xs font-medium px-2 py-1 rounded-full ${dashboardSpotlight === "employee-of-month"
                                                                            ? "bg-blue-100 text-blue-600"
                                                                            : "bg-gray-100 text-gray-600"
                                                                            }`}
                                                                    >
                                                        ...
                                                        return (
                                                            <>
                                                                <button
                                                                    type="button"
                                                                    onClick={() =>
                                                                        setDashboardSpotlight(
                                                                            "manager-monthly-ranking",
                                                                        )
                                                                    }
                                                                    className={`bg-white p-6 rounded-2xl shadow-sm border-2 transition-all duration-200 hover:shadow-md ${dashboardSpotlight ===
                                                                        "manager-monthly-ranking"
                                                                        ? "border-blue-500 shadow-lg shadow-blue-50"
                                                                        : "border-transparent hover:border-gray-200"
                                                                        }`}
                                                                        ...
                                                                        <div>
                                                                            <h2 className="text-sm font-semibold text-gray-900">
                                                                                <span className="inline-flex items-center gap-2">
                                                                                    ...
                                                                                    <span>
                                                                                        Employee of the Month Marketer
                                                                                    </span>
                                                                                </span>
                                                                            </h2>
                                                                            <p className="text-xs text-gray-500">
                                                                                Assign vs Achieved (month wise)
                                                                            </p>
                                                                        </div>
                                                                        <span
                                                                            className={`text-xs font-medium px-2 py-1 rounded-full ${dashboardSpotlight ===
                                                                                "manager-monthly-ranking"
                                                                                ? "bg-blue-100 text-blue-600"
                                                                                : "bg-gray-100 text-gray-600"
                                                                                }`}
                                                                ...
                                                                <button
                                                                    type="button"
                                                                    onClick={() =>
                                                                        setDashboardSpotlight("power-star-of-month")
                                                                    }
                                                                ...
                                                                >
                                                                    <div className="flex items-start justify-between">
                                                                        <div>
                                                                            ...
                                                                            <p className="text-xs text-gray-500 mt-1">
                                                                                Week wise (Churn / Live-Assign% / Hits)
                                                                            </p>
                                                                        </div>
                                                                        <span
                                                                            className={`text-xs font-medium px-2 py-1 rounded-full ${dashboardSpotlight ===
                                                ...
                                                >
                                                    <div className="flex justify-between items-start mb-4">
                                                        <div className="flex-1">
                                                            <div className="flex items-center gap-2 mb-3">
                                                                <span
                                                                    className={`px-3 py-1 text-xs font-medium rounded-full border ${getPriorityColor(task.priority || "medium")}`}
                                                                >
                                                                ...
                                                                <span
                                                                    className={`px-3 py-1 text-xs font-medium rounded-full border ${getStatusColor(task.status)}`}
                                                                >
                                                                    {task.status}
                                                            ...
                                                            <p className="text-sm text-gray-500 line-clamp-2 mb-3"></p>
                                                        </div>
                                                    </div>
                                                    <div className="space-y-3 mb-5">
                                                        ...
                                                        <div className="flex items-start justify-between text-sm gap-4">
                                                            ...
                                                            <span className="font-medium text-gray-900 text-right break-words">
                                                                {(() => {
                                                                    const assignedByUser: any = (task as any)
                                                                        ?.assignedByUser;
                                                                    const assignedBy: any = (task as any)
                                                                        ?.assignedBy;
                                                                    const email = (
                                                                        assignedByUser?.email ||
                                                                        (typeof assignedBy === "string"
                                                        ...
                                                        <div className="flex items-center justify-between text-sm">
                                                            ...
                                                            <span
                                                                className={`font-medium ${isOverdue(task.dueDate, task.status)
                                                                    ? "text-rose-600"
                                                                    : "text-gray-900"
                                                                    }`}
                                                            >
                                                        ...
                                                        <div className="flex items-center justify-between text-sm">
                                                            ...
                                                            <span
                                                                className={`px-2 py-1 text-xs rounded-full border ${getCompanyColor(task.companyName)}`}
                                                            >
                                                                {task.companyName}
                                                            </span>
                                                        </div>
                                                        {task.brand && (
                                                            <div className="flex items-center justify-between text-sm">
                                                                ...
                                                                <span
                                                                    className={`px-2 py-1 text-xs rounded-full border ${getBrandColor(task.brand)}`}
                                                                >
                                                    ...
                                                    <div className="flex gap-2 pt-4 border-t border-gray-100">
                                                        ...
                                                        {isSbmUser ? (
                                                            <button
                                                                type="button"
                                                                onClick={() =>
                                                                    handleOpenTaskCommentSidebar(task)
                                                                }
                                                                title="Comments"
                                                    ...
                                                    <tbody className="divide-y divide-gray-100">
                                                        {paginatedTasks.map((task: Task) => (
                                                            <tr key={task.id} className="hover:bg-gray-50">
                                                                ...
                                                                <td className="px-6 py-5">
                                                                    <span
                                                                        className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full border ${getStatusColor(task.status)}`}
                                                                    >
                                                                        {task.status}
                                                                    </span>
                                                                </td>
                                                                <td className="px-6 py-5">
                                                                    <span
                                                                        className={`inline-flex px-3 py-1 text-xs font-semibold rounded-full border ${getPriorityColor(task.priority || "medium")}`}
                                                                    >
                                                                    ...
                                                                    <td className="px-6 py-5 text-right">
                                                                        {canEditTask(task) && (
                                                                            <button
                                                                                ...
                                                                                title={
                                                                                    Boolean(task?.completedApproval)
                                                                                        ? "Editing not allowed for permanently approved tasks"
                                                                                        : "Edit"
                                                                                }
                                                                            >
                                ...
                                <CalendarView
                                    ...
                                    handleToggleTaskStatus={async (
                                        taskId: string,
                                        currentStatus: TaskStatus,
                                    ) => {
                                        try {
                                            await handleToggleTaskStatus(
                                                taskId,
                                                currentStatus,
                                                false,
                                            );
                                ...
                                <UserProfilePage
                                    ...
                                    onUserUpdated={(next) => {
                                        ...
                                        try {
                                            ...
                                            const nextEmail = String((next as any)?.email || "")
                                                .trim()
                                                .toLowerCase();
                                            setUsers((prev) => {
                                                const list = Array.isArray(prev) ? prev : [];
                                                return list.map((u: any) => {
                                                    ...
                                                    const uemail = String(u?.email || "")
                                                        ...
                                                        .toLowerCase();
                                                    const matchById =
                                                        nextId && uid && uid === String(nextId);
                                                    const matchByEmail =
                                                        nextEmail && uemail && uemail === nextEmail;
            ...
            <AddTaskModal
                ...
                getAvailableBrandOptions={() => availableBrandsForNewTask}
            ...
            />
            <MdImpexAddTaskModal
                open={showMdImpexAddTaskModal}
                onClose={() => setShowMdImpexAddTaskModal(false)}
                newTask={newTask}
                formErrors={formErrors}
                onChange={(field, value) => {
                    startTransition(() => {
